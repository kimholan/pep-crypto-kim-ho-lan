...
class VerifiablePIPv2
        implements ASN1Encodable {

    byte[] rawInput
    final ObjectIdentifiers notationIdentifier = ObjectIdentifiers.ID_BSNK_POLYMORPHIC_PIP_VERIFIABLE_V2
    SignedPIPv2 signedPIP
    VerifiablePIPv2ProofOfConformity proofOfConformity
    ...
    VerifiablePIPv2 decodeByteArray(byte[] bytes) {
        def target = this
    ...
    ASN1Encodable notationIdentifierReadRequired(VerifiablePIPv2 target, DERSequenceParser parser, ASN1Encodable object) {
        final ObjectIdentifiers notationIdentifier = ObjectIdentifiers.ID_BSNK_POLYMORPHIC_PIP_VERIFIABLE_V2

        if (!ObjectIdentifiers.ID_BSNK_POLYMORPHIC_PIP_VERIFIABLE_V2.toASN1Primitive().equals(object)) {
            throw new ASN1ParsingException("Expected object identifier: " + ObjectIdentifiers.ID_BSNK_POLYMORPHIC_PIP_VERIFIABLE_V2 + ", actual value " + object)
        }

        return readObject(parser)
    }

    ASN1Encodable signedPIPReadRequired(VerifiablePIPv2 target, DERSequenceParser parser, ASN1Encodable object) {
        def encoded = object.encoded
        def value = new SignedPIPv2()
    ...
    ASN1Encodable proofOfConformityReadRequired(VerifiablePIPv2 target, DERSequenceParser parser, ASN1Encodable object) {
        def encoded = object.encoded
        def value = new VerifiablePIPv2ProofOfConformity()
