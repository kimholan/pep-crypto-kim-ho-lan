...
class SignedEncryptedPseudonymv2
        implements ASN1Encodable, nl.logius.pepcrypto.lib.asn1.signedencryptedpseudonymv2.Asn1SignedEncryptedPseudonymv2Envelope {

    byte[] rawInput
    final ObjectIdentifiers notationIdentifier = ObjectIdentifiers.ID_BSNK_ENCRYPTED_PSEUDONYM_ECSDSA_SIGNED_V2
    SignedEncryptedPseudonymv2SignedEP signedEP
    ...
    SignedEncryptedPseudonymv2 decodeByteArray(byte[] bytes) {
        def target = this
    ...
    ASN1Encodable notationIdentifierReadRequired(SignedEncryptedPseudonymv2 target, DERSequenceParser parser, ASN1Encodable object) {
        final ObjectIdentifiers notationIdentifier = ObjectIdentifiers.ID_BSNK_ENCRYPTED_PSEUDONYM_ECSDSA_SIGNED_V2

        if (!ObjectIdentifiers.ID_BSNK_ENCRYPTED_PSEUDONYM_ECSDSA_SIGNED_V2.toASN1Primitive().equals(object)) {
            throw new ASN1ParsingException("Expected object identifier: " + ObjectIdentifiers.ID_BSNK_ENCRYPTED_PSEUDONYM_ECSDSA_SIGNED_V2 + ", actual value " + object)
        }

        return readObject(parser)
    }

    ASN1Encodable signedEPReadRequired(SignedEncryptedPseudonymv2 target, DERSequenceParser parser, ASN1Encodable object) {
        def encoded = object.encoded
        def value = new SignedEncryptedPseudonymv2SignedEP()
    ...
    ASN1Encodable signatureValueReadRequired(SignedEncryptedPseudonymv2 target, DERSequenceParser parser, ASN1Encodable object) {
        def encoded = object.encoded
