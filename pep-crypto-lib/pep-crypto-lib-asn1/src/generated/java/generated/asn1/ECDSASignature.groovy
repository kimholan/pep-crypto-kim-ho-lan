...
class ECDSASignature
    ...
    byte[] rawInput
    final ObjectIdentifiers signatureType = ObjectIdentifiers.ECDSA_WITH_SHA384
    ...
    ECDSASignature decodeByteArray(byte[] bytes) {
        def target = this
    ...
    ASN1Encodable signatureTypeReadRequired(ECDSASignature target, DERSequenceParser parser, ASN1Encodable object) {
        final ObjectIdentifiers signatureType = ObjectIdentifiers.ECDSA_WITH_SHA384

        if (!ObjectIdentifiers.ECDSA_WITH_SHA384.toASN1Primitive().equals(object)) {
            throw new ASN1ParsingException("Expected object identifier: " + ObjectIdentifiers.ECDSA_WITH_SHA384 + ", actual value " + object)
        }

        return readObject(parser)
    }

    ASN1Encodable signatureValueReadRequired(ECDSASignature target, DERSequenceParser parser, ASN1Encodable object) {
