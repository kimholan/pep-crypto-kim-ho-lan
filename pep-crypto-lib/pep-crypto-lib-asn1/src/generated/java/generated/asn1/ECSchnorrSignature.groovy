...
class ECSchnorrSignature
    ...
    byte[] rawInput
    final ObjectIdentifiers signatureType = ObjectIdentifiers.ECSCHNORR_PLAIN_SHA384
    ...
    ECSchnorrSignature decodeByteArray(byte[] bytes) {
        def target = this
    ...
    ASN1Encodable signatureTypeReadRequired(ECSchnorrSignature target, DERSequenceParser parser, ASN1Encodable object) {
        final ObjectIdentifiers signatureType = ObjectIdentifiers.ECSCHNORR_PLAIN_SHA384

        if (!ObjectIdentifiers.ECSCHNORR_PLAIN_SHA384.toASN1Primitive().equals(object)) {
            throw new ASN1ParsingException("Expected object identifier: " + ObjectIdentifiers.ECSCHNORR_PLAIN_SHA384 + ", actual value " + object)
        }

        return readObject(parser)
    }

    ASN1Encodable signatureValueReadRequired(ECSchnorrSignature target, DERSequenceParser parser, ASN1Encodable object) {
