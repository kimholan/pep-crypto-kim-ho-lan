...
class SignedPIPv2
    ...
    byte[] rawInput
    final ObjectIdentifiers notationIdentifier = ObjectIdentifiers.ID_BSNK_POLYMORPHIC_PIP_SIGNED_V2
    SignedPIPv2SignedPIP signedPIP
    ...
    SignedPIPv2 decodeByteArray(byte[] bytes) {
        def target = this
    ...
    ASN1Encodable notationIdentifierReadRequired(SignedPIPv2 target, DERSequenceParser parser, ASN1Encodable object) {
        final ObjectIdentifiers notationIdentifier = ObjectIdentifiers.ID_BSNK_POLYMORPHIC_PIP_SIGNED_V2

        if (!ObjectIdentifiers.ID_BSNK_POLYMORPHIC_PIP_SIGNED_V2.toASN1Primitive().equals(object)) {
            throw new ASN1ParsingException("Expected object identifier: " + ObjectIdentifiers.ID_BSNK_POLYMORPHIC_PIP_SIGNED_V2 + ", actual value " + object)
        }

        return readObject(parser)
    }

    ASN1Encodable signedPIPReadRequired(SignedPIPv2 target, DERSequenceParser parser, ASN1Encodable object) {
        def encoded = object.encoded
        def value = new SignedPIPv2SignedPIP()
    ...
    ASN1Encodable signatureValueReadRequired(SignedPIPv2 target, DERSequenceParser parser, ASN1Encodable object) {
        def encoded = object.encoded
