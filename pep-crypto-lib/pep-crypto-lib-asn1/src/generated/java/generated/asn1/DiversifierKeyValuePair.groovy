...
class DiversifierKeyValuePair
    ...
    ASN1Primitive toASN1Primitive() {
        ...
        items.add(new DERIA5String(value))
    ...
    DiversifierKeyValuePair decodeByteArray(byte[] bytes) {
        def target = this
    ...
    ASN1Encodable keyReadRequired(DiversifierKeyValuePair target, DERSequenceParser parser, ASN1Encodable object) {
        target.key = object.string

        return readObject(parser)
    }

    ASN1Encodable valueReadRequired(DiversifierKeyValuePair target, DERSequenceParser parser, ASN1Encodable object) {
        target.value = object.string
