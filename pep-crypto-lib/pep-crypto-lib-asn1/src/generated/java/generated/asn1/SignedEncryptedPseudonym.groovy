...
class SignedEncryptedPseudonym
        implements ASN1Encodable, nl.logius.pepcrypto.lib.asn1.signedencryptedpseudonym.Asn1SignedEncryptedPseudonymEnvelope {

    byte[] rawInput
    final ObjectIdentifiers notationIdentifier = ObjectIdentifiers.ID_BSNK_ENCRYPTED_PSEUDONYM_ECSDSA_SIGNED
    SignedEncryptedPseudonymSignedEP signedEP
    ...
    SignedEncryptedPseudonym decodeByteArray(byte[] bytes) {
        def target = this
    ...
    ASN1Encodable notationIdentifierReadRequired(SignedEncryptedPseudonym target, DERSequenceParser parser, ASN1Encodable object) {
        final ObjectIdentifiers notationIdentifier = ObjectIdentifiers.ID_BSNK_ENCRYPTED_PSEUDONYM_ECSDSA_SIGNED

        if (!ObjectIdentifiers.ID_BSNK_ENCRYPTED_PSEUDONYM_ECSDSA_SIGNED.toASN1Primitive().equals(object)) {
            throw new ASN1ParsingException("Expected object identifier: " + ObjectIdentifiers.ID_BSNK_ENCRYPTED_PSEUDONYM_ECSDSA_SIGNED + ", actual value " + object)
        }

        return readObject(parser)
    }

    ASN1Encodable signedEPReadRequired(SignedEncryptedPseudonym target, DERSequenceParser parser, ASN1Encodable object) {
        def encoded = object.encoded
        def value = new SignedEncryptedPseudonymSignedEP()
    ...
    ASN1Encodable signatureValueReadRequired(SignedEncryptedPseudonym target, DERSequenceParser parser, ASN1Encodable object) {
        def encoded = object.encoded
