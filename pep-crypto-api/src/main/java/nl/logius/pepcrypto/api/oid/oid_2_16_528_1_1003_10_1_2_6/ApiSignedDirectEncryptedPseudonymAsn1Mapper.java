...
public class ApiSignedDirectEncryptedPseudonymAsn1Mapper
        implements ApiAsn1Mapper<Asn1SignedDirectEncryptedPseudonymEnvelope> {

    @Override
    @ApiEventSource(ASN1_MAPPING)
    @ApiExceptionDetailMapper(ASN1_MAPPING_FAILED)
    public void processRawInput(ApiExchange<Asn1SignedDirectEncryptedPseudonymEnvelope> exchange) {
        var decodedAsn1Sequence = exchange.getRawInput();
        var signedDirectEncryptedPseudonym = Asn1SignedDirectEncryptedPseudonymEnvelope.fromByteArray(decodedAsn1Sequence);

        exchange.setMappedInput(signedDirectEncryptedPseudonym);
    }
